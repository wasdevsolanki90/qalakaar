...
const query = `
  *[_type == "returnAndExchangePolicyPage" && !(_id in path("drafts.**"))] {
    mainHeading,
    effectiveDate,
    introduction,
    sections[] {
...
export default async function ReturnAndExchangePolicyPage() {
  ...
  const policyData: PolicyData = data[0]; // Assuming there's only one policy document
  ...
  const { mainHeading, effectiveDate, introduction, sections, closingNote } = policyData;
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto  px-6">
      ...
      <p className="mt-8 text-lg">{closingNote}</p>
    </div>
    </section>
  );
};